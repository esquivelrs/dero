...
void ScEkfDero::ErrorCorrection(const VecXd &error_state_) {
  ...
  Vec3d euler_angles = quat2euler(state.quaternion);

  // Define maximum allowable roll and pitch angles (in radians)
  const double max_roll  = DEG2RAD(5.0);  // Maximum roll angle (adjust as needed)
  const double max_pitch = DEG2RAD(5.0);  // Maximum pitch angle (adjust as needed)

  // Clamp roll and pitch
  euler_angles(0) = std::max(std::min(euler_angles(0), max_roll), -max_roll);
  euler_angles(1) = std::max(std::min(euler_angles(1), max_pitch), -max_pitch);

  // Reconstruct the quaternion from the clamped Euler angles
  state.quaternion = euler2quat(euler_angles);
