...
void RunDeRO::Run() {
  ...
  std::lock_guard<std::mutex> radar_lock(radar_mutex_);
  if (!queue_radar_buff.empty() && !imu_only && use_radar) {
    ...
    if (!ekf_rio_init_) {
      ...
      RCLCPP_INFO(this->get_logger(), "radar info: %s", radar_estimator_.getRadarInfo().c_str());
      ...
      if (!use_dr_structure) {
        ...
        if (!accel_trigger) {
          ...
          RCLCPP_INFO(this->get_logger(), "v_r_r: [%.2f %.2f %.2f]", v_r_r(0, 0), v_r_r(1, 0), v_r_r(2, 0));
          ...
          RCLCPP_INFO(this->get_logger(), "Estimated accel: [%.2f %.2f %.2f]", estimated_accel(0, 0),
                                                                              estimated_accel(1, 0),
                                                                              estimated_accel(2, 0));
        }

        RCLCPP_INFO(this->get_logger(), "Radar: check point");
